...
logging.basicConfig(filename='/data02/sas2py_poc/act/ipm_rrc/ipm_rrc_hhldrtn_home/logs/ipm_rrc_cnclratio_regional_auto.log',
                    level=logging.INFO, format='%(asctime)s - '+'GW_IPM_HHLDRTN_HOME'+' %(levelname)s - %(message)s')

''' Creating temporary sqlite working DB to store all temporay stating results '''
SQLitePythonWorkDb = '/data02/sas2py_poc/act/ipm_rrc/ipm_rrc_hhldrtn_hom/data/ipm_rrc_hhldrtn_home.db'
...
HHLDRTN_HOMEDB = pd.read_sql_query(
    "select * from NFRCOUT_HHLDRTN_HOMEDB ", sqliteConnection)
df_lower_colNames(HHLDRTN_HOMEDB, 'NFRCOUT_HHLDRTN_HOMEDB')
