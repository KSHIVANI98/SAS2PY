...
logging.basicConfig(filename='/data02/sas2py_poc/act/ipm_rrc/ipm_rrc_cnclhome/code/ipm_rrc_cnclhome.log',
                    level=logging.INFO, format='%(asctime)s - '+'GW_IPM_CNCLHOME'+' %(levelname)s - %(message)s')

''' Creating temporary sqlite working DB to store all temporay stating results '''
SQLitePythonWorkDb = '/data02/sas2py_poc/act/ipm_rrc/ipm_rrc_cnclhome/data/ipm_rrc_cnclhome.db'
...
# TODO - Remove extra spaces from the end
...
# TODO - Why using IUT015M_2102 instead of IUT015M?
# file_str = open(
    # r"/data02/sas2py_poc/act/ipm_rrc/ipm_rrc_hhldrtn/data/IUT015M_2102.TXT").read()
...
# curr_ccyymm = datetime.datetime.strptime(mnthnd_ccyymm, "%Y%m").strftime("%Y%m").strip()
# curr_ccyy = datetime.datetime.strptime(mnthnd_ccyy, "%Y").strftime("%Y").strip()
...
# # handling data frame column case senstivity.#
# df_lower_colNames(df, 'mihmcnv_{}'.format(amnthago))
...
# # handling data frame column case senstivity.#
# df_lower_colNames(df, 'mihmcnv_{}'.format(curr_ccyymm))
# # Push results data frame to Sqlite DB
...
# # handling data frame column case senstivity.#
# df_lower_colNames(df, 'rgnhmcnv_{}'.format(amnthago))
# df['nfrccnt1'] = df.nfrccnt
# df['system'] = df.syscd
# df['mnthnd'] = curr_ccyymm
# # Push results data frame to Sqlite DB
...
# cmb_nfrcrtnd.loc[(cmb_nfrcrtnd.rnwgcnt == 0) & (cmb_nfrcrtnd.rtncnt1 == 1), 'rtncnt2'] = df.nfrccnt
...
# df_cmb_nfrcrtnd = df.copy()

# df_unknown = df.loc[df.agttyp == 'Unknown']
# df_unknown['agttyp'] = 'HB'
# df_unknown['valseq'] = 3
...
# df = df_cmb_nfrcrtnd.copy()
# df = df.loc[df['agttyp'].isin(['SSA', 'EA'])]
# df['agtsave'] = df['agttyp']
# df['agttyp'] = 'Captive/EA'
# df = df_remove_indexCols(df)
# df.to_sql("homenfrc", con=sqliteConnection, if_exists='append')
# # TODO - Is it before or after writing to sql?
# df['agttyp'] = df['agtsave']
# del df

# df = df_cmb_nfrcrtnd.copy()

# df = df.loc[df['agttyp'].isin(['MSC', 'HB'])]
# df['agtsave'] = df['agttyp']
# df['agttyp'] = 'MSC/HB'
# df = df_remove_indexCols(df)
# df.to_sql("homenfrc", con=sqliteConnection, if_exists='append')
...
# df['prind'] = df['prind'].astype(str)
# df['n_1'] = df['prind'].apply(NONMI_MULTI_SEQ)
# df['m_1'] = df['prind'].apply(MI_MULTI_SEQ)
# df['dimval'] = df.apply(lambda x: x['m_1'] if x['state']
...
# df = df.loc[(df['system'] == 'I') & (df.form == 'H4')]
# df['dimval'] = 'Other'
# df['valseq'] = 7
# df = df_remove_indexCols(df)
# df.to_sql("homenfrc", con=sqliteConnection, if_exists='append')
...
# df['valseq'] = df['rfage'].apply(RFAGE_SEQ)
# df['dimval'] = df['valseq'].apply(RFAGE_VAL)
# df = df_remove_indexCols(df)
# df.to_sql("homenfrc", con=sqliteConnection, if_exists='append')
# del df
# df = df_cmb_nfrcrtnd.copy()

# df = df.loc[(df['agttyp'].isin(['SSA', 'EA'])) & (df.system == 'I')]
# df['agtsave'] = df['agttyp']
# df['agttyp'] = 'Captive/EA'
# df = df_remove_indexCols(df)
# df.to_sql("homenfrc", con=sqliteConnection, if_exists='append')
...
# df = df.loc[(df['agttyp'].isin(['MSC', 'HB'])) & (df.system == 'I')]
# df['agtsave'] = df['agttyp']
# df['agttyp'] = 'MSC/HB'
# df = df_remove_indexCols(df)
# df.to_sql("homenfrc", con=sqliteConnection, if_exists='append')
...
# df['dim'] = 'Membership'
# df['dimseq'] = 6
# df['valseq'] = df['mbrind'].apply(MBRIND_SEQ)
# df['dimval'] = df['valseq'].apply(MBRIND_VAL)

# # df['agtsave']=df['agttyp']
# # df['agttyp']='MSC/HB'
# col = 'agttyp'
# conditions = [df[col].isin(['SSA', 'EA']), df[col].isin(['MSC', 'HB'])]
# choices1 = [df['agttyp'], df['agttyp']]
# choices2 = ['Captive/EA', 'MSC/HB']
# df['agtsave'] = np.select(conditions, choices1)
# df['agttyp'] = np.select(conditions, choices2)
# df = df_remove_indexCols(df)
# df.to_sql("homenfrc", con=sqliteConnection, if_exists='append')
# df['agttyp'] = df['agtsave']
# del df
...
# df = df_remove_indexCols(df)
# df.to_sql("homenfrc",con=sqliteConnection,if_exists='append',index=True)
# del df
...
# col = 'totlossa'
# col1 = 'totlossb'
# conditions = [(df[col] > 0) & (df[col1] > 0), (df[col] > 0) &
               # (df[col1] == 0), (df[col] == 0) & (df[col1] > 0)]
# choices1 = [3, 1, 2]
# choices2 = ['Both', 'A', 'B']
# df['valseq'] = np.select(conditions, choices1)
# df['dimval'] = np.select(conditions, choices2)
...
# df['valseq'] = 2
# df['dimval'] = 'No'
# df = df.loc[(df.totlossa > 0) | (df.totlossb > 0)]
# df['valseq'] = 1
# df['dimval'] = 'Yes'
# df = df.loc[df.system == 'I']
# df = df_remove_indexCols(df)
...
# df_0_summ = df_0[['nfrccnt1', 'rtncnt1', 'nfrccnt2', 'rtncnt2']].sum()

# for i in range(1, len(grp_lst)+1):
    # logging.info('Running proc summary')
    # for j in itertools.combinations(grp_lst, i):
        # cnt = cnt +1
        # df1 = df.groupby(list(j))[['nfrccnt1','rtncnt1', 'nfrccnt2', 'rtncnt2']].sum().reset_index()
        # df2 = df.sort_values(list(j)+id_list)
        # df2 = df2[list(j)+id_list]
        # df2['IsFirst'] ,df2['IsLast'] = [False, False]
        # df2.loc[df2.groupby(list(j) + id_list)['IsFirst'].head(1).index, 'IsFirst'] = True
        # df2.loc[df2.groupby(list(j) + id_list)['IsLast'].tail(1).index, 'IsLast'] = True
        # df2 = df2[df2['IsLast']]
...
# grouped_df = grouped_df[grp_lst+id_list + ['nfrccnt1','rtncnt1', 'nfrccnt2', 'rtncnt2']]
...
# grp_lst = ['mnthnd', 'system', 'product','state','agttyp','dimseq'] #Take the columns in Class
...
# sqliteConnection = sqlite3.connect(SQLitePythonWorkDb)
# df = pd.read_sql_query("select * from SUMDB", sqliteConnection)
# # handling data frame column case senstivity.#
# df_lower_colNames(df, 'SUMDB')
# df=df.loc[~df['agttyp'].isin(['SSA', 'EA', 'MSC', 'HB'])]
# grp_lst = ['mnthnd', 'system', 'product','state','agttyp','dimseq','valseq'] #Take the columns in Class
# id_list = ['dim','dimval'] # Take the columns in ID
# grouped_df = pd.DataFrame()
# cnt = 0
# df_0 = df.drop(columns = grp_lst)
...
# Open connection to Sqlite work data base
sqliteConnection = sqlite3.connect(SQLitePythonWorkDb)
# Converting source work.nfrcrtnd data into datafram.
nfrcrtnd = pd.read_sql_query("select * from nfrcrtnd ",sqliteConnection)
df_lower_colNames(nfrcrtnd) #lowering all column names#Generate first and last temporary indicators in the given data frames.
nfrcrtnd_grouped = nfrcrtnd
nfrcrtnd_grouped = nfrcrtnd_grouped.sort_values(by = ['mnthnd','system','state','agttyp','dimseq','valseq'])
nfrcrtnd_grouped['IsFirst'] ,nfrcrtnd_grouped['IsLast'] = [False, False]
nfrcrtnd_grouped.loc[nfrcrtnd_grouped.groupby(['mnthnd','system','state','agttyp','dimseq','valseq'])['IsFirst'].head(1).index, 'IsFirst'] = True
nfrcrtnd_grouped.loc[nfrcrtnd_grouped.groupby(['mnthnd','system','state','agttyp','dimseq','valseq'])['IsLast'].tail(1).index, 'IsLast'] = True
# Output first occurance values in data to the target data frame.
fxdnfrc = nfrcrtnd_grouped[(nfrcrtnd_grouped['IsFirst'])]# Drop indicator tmp columns
fxdnfrc = fxdnfrc.drop(columns=['IsFirst','IsLast'])
nfrcrtnd_grouped.loc[nfrcrtnd_grouped.groupby(['dimseq'])['IsFirst'].head(1).index, 'frstdim'] = True
# Push results data frame to Sqlite DB
...
sqliteConnection = sqlite3.connect(SQLitePythonWorkDb)
# Converting source df data into datafram.
df = pd.read_sql_query("select * from cncldb.cnclrat_homedb ",sqliteConnection)
# handling data frame column case senstivity.#
df_lower_colNames(df,'cncldb.cnclrat_homedb')
# Push results data frame to Sqlite DB
logging.info("cncldb.cnclrat_homedb_backup created successfully with {} records".format(df))
df.to_sql("cncldb.cnclrat_homedb_backup",con=sqliteConnection,if_exists='replace')
# Close connection to Sqlite work data base
sqliteConnection.close()
#*******************************End of Data Step Process**************************************************#
...
cncldb.cnclrat_homedb = pd.read_sql_query("select * from cncldb.cnclrat_homedb ",sqliteConnection)
# Converting source work.fxdnfrc data into datafram.
fxdnfrc = pd.read_sql_query("select * from fxdnfrc ",sqliteConnection)
# Concatenate the source data frames
cncldb = pd.concat([cncldb.cnclrat_homedb,fxdnfrc],ignore_index=True,sort=False)
# Push results data frame to Sqlite DB
df = df_remove_indexCols(cncldb)
df_creation_logging(cncldb)
...
# Push results data frame to Sqlite DB
logging.info("cncldb.cnclrat_homedb created successfully with {} records".format(len(df)))
df.to_sql("cncldb.cnclrat_homedb",con=sqliteConnection,if_exists='replace')
# Close connection to Sqlite work data base
sqliteConnection.close()
#*******************************End of Data Step Process**************************************************#
...
cmb_nfrcrtnd = pd.read_sql_query("SELECT nfrccnt1,rtncnt1,nfrccnt2,rtncnt2 FROM cmb_nfrcrtnd; ",sqliteConnection)
# Close connection to Sqlite work data base
sqliteConnection.close()
# Creating PRCO FREQ results in python for source column :NFRCCNT1.
nfrccnt1_freq = cmb_nfrcrtnd['nfrccnt1'].value_counts().to_frame().reset_index().rename(columns = {'nfrccnt1':'Frequency', 'index' :'nfrccnt1'})
nfrccnt1_freq["Percent"] = 100*nfrccnt1_freq['Frequency']/nfrccnt1_freq['Frequency'].sum()
nfrccnt1_freq['Cumulative Frequency'] = nfrccnt1_freq['Frequency'].cumsum()
nfrccnt1_freq['cum_perc'] = 100*nfrccnt1_freq['Cumulative Frequency']/nfrccnt1_freq['Frequency'].sum()

# Push results data frame to Sqlite DB
df = df_remove_indexCols(nfrccnt1_freq)
df_creation_logging(nfrccnt1_freq)
nfrccnt1_freq.to_sql("nfrccnt1_freq",con=sqliteConnection,if_exists='replace')

# Creating PRCO FREQ results in python for source column :RTNCNT1.
rtncnt1_freq = cmb_nfrcrtnd['rtncnt1'].value_counts().to_frame().reset_index().rename(columns = {'rtncnt1':'Frequency', 'index' :'rtncnt1'})
rtncnt1_freq["Percent"] = 100*rtncnt1_freq['Frequency']/rtncnt1_freq['Frequency'].sum()
rtncnt1_freq['Cumulative Frequency'] = rtncnt1_freq['Frequency'].cumsum()
rtncnt1_freq['cum_perc'] = 100*rtncnt1_freq['Cumulative Frequency']/rtncnt1_freq['Frequency'].sum()

# Push results data frame to Sqlite DB
df = df_remove_indexCols(rtncnt1_freq)
df_creation_logging(rtncnt1_freq)
rtncnt1_freq.to_sql("rtncnt1_freq",con=sqliteConnection,if_exists='replace')

# Creating PRCO FREQ results in python for source column :NFRCCNT2.
nfrccnt2_freq = cmb_nfrcrtnd['nfrccnt2'].value_counts().to_frame().reset_index().rename(columns = {'nfrccnt2':'Frequency', 'index' :'nfrccnt2'})
nfrccnt2_freq["Percent"] = 100*nfrccnt2_freq['Frequency']/nfrccnt2_freq['Frequency'].sum()
nfrccnt2_freq['Cumulative Frequency'] = nfrccnt2_freq['Frequency'].cumsum()
nfrccnt2_freq['cum_perc'] = 100*nfrccnt2_freq['Cumulative Frequency']/nfrccnt2_freq['Frequency'].sum()

# Push results data frame to Sqlite DB
df = df_remove_indexCols(nfrccnt2_freq)
df_creation_logging(nfrccnt2_freq)
nfrccnt2_freq.to_sql("nfrccnt2_freq",con=sqliteConnection,if_exists='replace')

# Creating PRCO FREQ results in python for source column :RTNCNT2.
rtncnt2_freq = cmb_nfrcrtnd['rtncnt2'].value_counts().to_frame().reset_index().rename(columns = {'rtncnt2':'Frequency', 'index' :'rtncnt2'})
rtncnt2_freq["Percent"] = 100*rtncnt2_freq['Frequency']/rtncnt2_freq['Frequency'].sum()
rtncnt2_freq['Cumulative Frequency'] = rtncnt2_freq['Frequency'].cumsum()
rtncnt2_freq['cum_perc'] = 100*rtncnt2_freq['Cumulative Frequency']/rtncnt2_freq['Frequency'].sum()

# Push results data frame to Sqlite DB
df_creation_logging(rtncnt2_freq)
rtncnt2_freq.to_sql("rtncnt2_freq",con=sqliteConnection,if_exists='replace')
...
# Push results data frame to Sqlite DB
df = df_remove_indexCols(rtncnt2_freq)
df_creation_logging(rtncnt2_freq)
rtncnt2_freq.to_sql("rtncnt2_freq",con=sqliteConnection,if_exists='replace')
