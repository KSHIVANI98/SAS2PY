...
try:
    config_file = open(
        '/data02/sas2py_poc/act/ipm_rrc/ipm_rrc_cnclratio_regional_auto/config.yaml', 'r+')
...
log_file_name = os.path.basename(__file__)[len('sas2py_converted_script_'):-3]
logging.basicConfig(filename=yaml_file['logs'] + os.sep + log_file_name + '.log', level=logging.INFO,
                    format='%(asctime)s - ' + log_file_name.upper()+' %(levelname)s - %(message)s')
...
def sas_to_bq(tables_list, InfMon, dataset_id):
	client = bigquery.Client()
	for table in tables_list:
		logging.info('Reading dataset {} from GCP for {}'.format(table, InfMon))
		sql = """select count(*) from {}.{}.outfile_{} where mondate = {}""".format(
		    project_id, dataset_id, table, InfMon)
		count = client.query(sql).to_dataframe().iloc[0][0]
		if count != 0:
			logging.info('Dataset {} not found on GCP for {}'.format(table, InfMon))
			data_path = yaml_file['data']
			logging.info('Reading SAS dataset {} from data folder'.format(table + str(InfMon)))
			sql = """select column_name, data_type
					from `{}.{}.INFORMATION_SCHEMA.COLUMNS`
					where table_name = 'outfile_{}'""".format(project_id, dataset_id, table)
			col_df = client.query(sql).to_dataframe()
			bq_schema = []
			orig_bq_cols = sorted(col_df['column_name'].tolist())
			bq_date_cols = col_df.loc[col_df.data_type == 'TIMESTAMP']['column_name'].tolist()
			col_dict = dict(zip(col_df.column_name, col_df.data_type))
			for col in col_dict:
				bq_schema.append({'name':col, 'type':col_dict[col]})
			with SAS7BDAT('{}/{}.sas7bdat'.format(data_path, (table + str(InfMon)))) as reader:
				df = reader.to_data_frame()
				df['mondate'] = InfMon
				df.columns = [x.upper() if x.upper() in orig_bq_cols else x.lower() for x in df.columns]
				for i in bq_date_cols:
					print(df[i].unique())
					df[i] = pd.to_datetime(df[i].apply(str.title), format = '%d%b%Y', errors = 'coerce')
					print(df[i].unique())
				df_cols = sorted(list(df.columns))
				orig_col_dict = dict(zip(df_cols, orig_bq_cols))
				print(orig_col_dict)
				df = df.rename(columns = orig_col_dict)
				df.to_gbq(destination_table=(dataset_id + '.' + 'outfile_' + table), project_id=project_id, if_exists='append', table_schema = bq_schema)
		else:
			logging.info('Dataset {} found on GCP for {}'.format(table, InfMon))
...
def sqliteToBQ(output_tables):
    ...
    try:
        SQLitePythonWorkDb = yaml['workdb']
        sqliteConnection = sqlite3.connect(SQLitePythonWorkDb)
        df = pd.read_sql(sql, con=sqliteConnection)
        df.to_gbq(destination_table=output_dataset + '.' +
                  output_tables, project_id=project_id, if_exists='replace')
        logging.info('output_table {} created successfully with {} records and {} columns.'.format(
            output_tables.upper(), incr_recs, cols))
        del df
        gc.collect()
    except sqlite3.Error as e:
        logging.error('output_table {} creation is failed.'.format(
            output_tables.upper()))
...
# Exporting sas datasets to GCP if required
stuff = InfMon2
while(stuff < LatestMon):
    stuff = stuff+1
    if stuff % 100 == 13:
        stuff = stuff+88
    sas_to_bq(yaml_file['inforce_datasets'], stuff, gen1_dataset_id)
    # sas_to_bq(yaml_file['inforce_datasets'], stuff, gen2_dataset_id)
...
def GetPol(InfMon):
    ...
    *********************************************************************************'''
    ...
    client = bigquery.Client()
    sql = """select * from {}.{}.inforce where mondate = {}""".format(
        project_id, gen1_dataset_id, InfMon)
    ...
    sql = """select * from {}.{}.inforce where mondate = {}""".format(
        project_id, gen2_dataset_id, InfMon)
    ...
    df = df.sort_values(['mvstate', 'policy', 'hhclient']).drop_duplicates(
        ['mvstate', 'policy', 'hhclient'])
    # df = df_remove_indexCols(df)
    logging.info(
        "InfRen{} created successfully with {} records".format(InfMon, len(df)))
    # Push results data frame to Sqlite DB
...
def Renew(InfMon):
    ...
    *********************************************************************************'''
    # Please Note - If any library references remove them accordingly post your code analysis.#
    # Open connection to Sqlite work data base
    sql = """select * from {}.{}.driver where mondate = {}""".format(
        project_id, gen1_dataset_id, InfMon)
...
df_cnclratio_regautodb = client.query(sql.to_dataframe)
df_cnclratio_regautodb.to_gbq(destination_table = output_dataset + '.' + 'cnclratio_regautodb_backup', project_id = project_id, if_exists='replace')
...
cnclratio_regautodb = df_cnclratio_regautodb
df_lower_colNames(cnclratio_regautodb, 'cnclratio_regautodb')
cnclratio_regautodb['mnthnd'] = cnclratio_regautodb.mnthnd.astype('Int64')
cnclratio_regautodb = cnclratio_regautodb.loc[df.mnthnd < int(LatestMon)]
