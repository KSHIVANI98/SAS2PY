...
def sqliteToBQ(output_tables):
    i = 1
    incr_recs = 0
    sql = "select * from rnwldb"
    logging.info('Getting table rnwldb from sqlitedb')	
    try:
        SQLitePythonWorkDb = yaml_file['workDb']
        sqliteConnection = sqlite3.connect(SQLitePythonWorkDb)
...
def join_onemnth():
    ...
    *********************************************************************************'''
    ...
    df = df_rgnhmcnv2
    # handling data frame column case senstivity.#
    df_lower_colNames(df, 'rgnhmcnv_')
    # Push results data frame to Sqlite DB
    df = df_remove_indexCols(df)
    logging.info(
        "rgnl_nfrcrtnd_{} created successfully with {} records".format(curr_ccyymm, len(df)))
...
sql = """select * from {}.{}.rtnrnwl_homedb""".format(project_id, output_dataset)
df_rtnrnwl_homedb = client.query(sql).to_dataframe()
df_rtnrnwl_homedb.to_gbq(destination_table = output_dataset + '.' + 'rtnrnwl_homedb_backup', project_id = project_id, if_exists='replace')
'''
sqliteConnection = sqlite3.connect(SQLitePythonWorkDb)
...
'''nfrcout_rtnrnwl_homedb = pd.read_sql_query(
    "select * from nfrcout_rtnrnwl_homedb ", sqliteConnection)'''
nfrcout_rtnrnwl_homedb = df_rtnrnwl_homedb
df_lower_colNames(nfrcout_rtnrnwl_homedb, 'rtnrnwl_homedb')
nfrcout_rtnrnwl_homedb['mnthnd'] = nfrcout_rtnrnwl_homedb.mnthnd.astype('Int64')
nfrcout_rtnrnwl_homedb = nfrcout_rtnrnwl_homedb.loc[df.mnthnd < int(curr_ccyymm)]
